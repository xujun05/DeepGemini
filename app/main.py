...
from app.deepgemini.deepgemini import DeepGemini
...
GEMINI_API_URL = os.getenv(
    "GEMINI_API_URL", 
    "https://generativelanguage.googleapis.com/v1/models/gemini-exp-1206/versions/1:generate"
)
...
if not deep_claude and not deep_gemini:
    logger.critical("请至少设置 CLAUDE_API_KEY 或 GEMINI_API_KEY 其中之一")
    sys.exit(1)
...
async def chat_completions(request: Request):
    ...
    try:
        ...
        processor = None
        if "deepgemini" in model.lower() and deep_gemini:
            processor = deep_gemini
        elif deep_claude:  # 默认使用 DeepClaude
            processor = deep_claude
        else:
        ...
        if stream:
            return StreamingResponse(
                processor.chat_completions_with_stream(
                    messages=messages,
                    model_arg=model_arg[:4] if isinstance(processor, DeepClaude) else None,
                    deepseek_model=DEEPSEEK_MODEL,
                    claude_model=CLAUDE_MODEL if isinstance(processor, DeepClaude) else GEMINI_MODEL
                ),
